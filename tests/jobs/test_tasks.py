...
async def test_run_pipeline_generates_pdf_and_completes() -> None:
    ...
    # Each `async with async_session_maker()` call gets a fresh mock session
    ...
    with (
        patch("src.jobs.tasks.async_session_maker", return_value=mock_ctx),
        ...
        mock_update_status.assert_any_call(report_id, REPORT_STATUS_COMPLETED)
